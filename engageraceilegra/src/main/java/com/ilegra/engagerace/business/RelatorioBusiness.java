		...
		if(registrosRelatorio != null && registrosRelatorio.size() > 0){
			...
			for(RelatorioDto registro : registrosRelatorio){
				...
				if(tipo == 3){
					...
					cell1.setCellValue(registro.getData());
				}

				Cell cell2 = row.createCell(cellnum++);
				cell2.setCellValue(registro.getNomeusuario());

				Cell cell3 = row.createCell(cellnum++);
				cell3.setCellValue(registro.getArea());

				if (tipo == 1 || tipo == 3){
					Cell cell4 = row.createCell(cellnum++);
					cell4.setCellValue(registro.getPrograma());

					Cell cell5 = row.createCell(cellnum++);
					cell5.setCellValue(registro.getTipoprograma());
				}
